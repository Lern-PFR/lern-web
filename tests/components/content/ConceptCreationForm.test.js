...
import { ConceptCreationForm } from 'components/content';
...
describe('Concept creation form', () => {
	...
	describe('Snapshot testing', () => {
		const wrapper = shallow(<ConceptCreationForm onSubmit={jest.fn()} />);
	...
	describe('Form inputs', () => {
		it('should contain a "title" text input.', () => {
			const { container } = render(<ConceptCreationForm onSubmit={jest.fn()} />);
	...
	describe('Form inputs validation', () => {
		describe('title input validation', () => {
			...
			beforeEach(() => {
				...
				container = render(<ConceptCreationForm onSubmit={mockedOnSubmit} />).container;
			...
			describe('on submit validation', () => {
				it('should trigger a "required" validation error on the title input if it is empty on submit', () => {
					...
					expect(container.textContent).toMatch(new RegExp('concepts.creation.form.fields.title.validation_rules.required'));
				});
				...
				it('should trigger a "min_length" validation error on the title input if it has less than 3 characters on submit', () => {
					...
					expect(mockedOnSubmit).not.toHaveBeenCalled();
					expect(container.textContent).toMatch(new RegExp('concepts.creation.form.fields.title.validation_rules.min_length'));
				});
				...
				it('should trigger a "max_length" validation error on the title input if it has more than 50 characters on submit', () => {
					...
					expect(mockedOnSubmit).not.toHaveBeenCalled();
					expect(container.textContent).toMatch(new RegExp('concepts.creation.form.fields.title.validation_rules.max_length'));
				});
			});
			...
			describe('on change validation', () => {
				...
				it('should trigger a "required" validation error on the title input on change after the first submit', () => {
					act(() => {
						fireEvent.input(titleInput, { target: { value: '' } });
						expect(container.textContent).not.toMatch(new RegExp('concepts.creation.form.fields.title.validation_rules.required'));
						fireEvent.click(submitButton);
		...
		describe('description textarea validation', () => {
			...
			describe('on submit validation', () => {
				it('should trigger a "required" validation error on the description textarea if it is empty on submit', () => {
					...
					expect(mockedOnSubmit).not.toHaveBeenCalled();
					expect(container.textContent).toMatch(new RegExp('concepts.creation.form.fields.description.validation_rules.required'));
				});
				...
				it('should trigger a "min_length" validation error on the description textarea if it has less than 10 characters on submit', () => {
					...
					expect(mockedOnSubmit).not.toHaveBeenCalled();
					expect(container.textContent).toMatch(new RegExp('concepts.creation.form.fields.description.validation_rules.min_length'));
				});
				...
				it('should trigger a "max_length" validation error on the description textarea if it has more than 301 characters on submit', () => {
					...
					expect(mockedOnSubmit).not.toHaveBeenCalled();
					expect(container.textContent).toMatch(new RegExp('concepts.creation.form.fields.description.validation_rules.max_length'));
				});
			});
			...
			describe('on change validation', () => {
				...
				it('should trigger a "required" validation error on the description input on change after the first submit', () => {
					act(() => {
						fireEvent.input(descriptionTextarea, { target: { value: '' } });
						expect(container.textContent).not.toMatch(new RegExp('concepts.creation.form.fields.description.validation_rules.required'));
						fireEvent.click(submitButton);
	...
	describe('onSubmit', () => {
		it('should call the onSubmit prop method with the form\'s inputs\' values.', async () => {
			...
			const { container } = render(<ConceptCreationForm onSubmit={mockedOnSubmit} />);
