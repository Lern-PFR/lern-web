...
/**
 * @name ConceptCreationForm
 * @description A form component used to create a concept.
 *
 * @author Timothée Simon-Franza
 *
 * @param {func}	onSubmit	The method to trigger on form submit.
 */
const ConceptCreationForm = ({ onSubmit }) => {
	...
	const getErrorMessageByFieldName = (fieldName) => {
		if (errors[fieldName] && Object.keys(errors[fieldName])?.length > 0) {
			return t(`concepts.creation.form.fields.${fieldName}.validation_rules.${Object.values(errors[fieldName])?.[0]}`);
		}
	...
	return (
		<StyledForm onSubmit={handleSubmit} {...form}>
				...
				inputType === 'textarea'
					? (
						<LabeledTextArea
							...
							placeholder={hasPlaceholder ? t(`concepts.creation.form.fields.${name}.placeholder`) : undefined}
							ref={(fieldRef) => { fieldsRef.current[name] = fieldRef; }}
							defaultValue={formState[name]}
						>
							{t(`concepts.creation.form.fields.${name}.label`)}
						</LabeledTextArea>
					)
					: (
						<LabeledInput
			...
			<PrimaryButton type="submit" {...formSubmit}>
				{t('concepts.creation.form.action.submit')}
			</PrimaryButton>
		</StyledForm>
	);
};

ConceptCreationForm.propTypes = {
