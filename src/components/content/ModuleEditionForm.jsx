...
/**
 * @name ModuleEditionForm
 * @description A form component used to edit a module.
 *
 * @author Timothée Simon-Franza
 *
 * @param {object}	module				The module to edit.
 * @param {array}	moduleOrderOptions	Options to provide the order select input.
 * @param {func}	onSubmit			The method to trigger on form submission.
 */
const ModuleEditionForm = ({ module, moduleOrderOptions, onSubmit }) => {
	const initialState = useMemo(() => ({
		title: module.title,
		description: module.description,
		order: (module.order !== null && module.order !== undefined) ? { label: module.order, value: module.order } : null,
	}), [module]);
	...
	const getErrorMessageByFieldName = (fieldName) => {
		if (errors[fieldName] && Object.keys(errors[fieldName])?.length > 0) {
			return t(`modules.edition.form.fields.${fieldName}.validation_rules.${Object.values(errors[fieldName])?.[0]}`);
		}
	...
	return (
		<StyledForm onSubmit={handleSubmit} {...form}>
					...
					case 'textarea': {
						return (
							<LabeledTextArea
								...
								placeholder={hasPlaceholder ? t(`modules.edition.form.fields.${name}.placeholder`) : undefined}
								ref={(fieldRef) => { fieldsRef.current[name] = fieldRef; }}
								defaultValue={formState[name]}
							>
								{t(`modules.edition.form.fields.${name}.label`)}
							</LabeledTextArea>
						);
					}
					case 'select': {
			...
			<PrimaryButton type="submit" {...formSubmit}>
				{t('modules.edition.form.action.submit')}
			</PrimaryButton>
		</StyledForm>
	);
};

ModuleEditionForm.propTypes = {
	module: PropTypes.shape({
		title: PropTypes.string.isRequired,
		description: PropTypes.string.isRequired,
		order: PropTypes.number.isRequired,
	}).isRequired,
	moduleOrderOptions: PropTypes.arrayOf(
		PropTypes.shape({
...
export default ModuleEditionForm;
