...
/**
 * @name ConceptEditionForm
 * @description A form component used to edit a concept.
 *
 * @author Timothée Simon-Franza
 *
 * @param {object}	concept				The concept to edit.
 * @param {array}	conceptOrderOptions	Options to provide the order select input.
 * @param {func}	onSubmit			The method to trigger on form submission.
 */
const ConceptEditionForm = ({ concept, conceptOrderOptions, onSubmit }) => {
	const initialState = useMemo(() => ({
		title: concept.title,
		description: concept.description,
		order: (concept.order !== null && concept.order !== undefined) ? { label: concept.order, value: concept.order } : null,
	}), [concept]);
	...
	const getErrorMessageByFieldName = (fieldName) => {
		if (errors[fieldName] && Object.keys(errors[fieldName])?.length > 0) {
			return t(`concepts.edition.form.fields.${fieldName}.validation_rules.${Object.values(errors[fieldName])?.[0]}`);
		}
	...
	return (
		<StyledForm onSubmit={handleSubmit} {...form}>
					...
					case 'textarea': {
						return (
							<LabeledTextArea
								...
								placeholder={hasPlaceholder ? t(`concepts.edition.form.fields.${name}.placeholder`) : undefined}
								ref={(fieldRef) => { fieldsRef.current[name] = fieldRef; }}
								defaultValue={formState[name]}
							>
								{t(`concepts.edition.form.fields.${name}.label`)}
							</LabeledTextArea>
						);
					}
					case 'select': {
			...
			<PrimaryButton type="submit" {...formSubmit}>
				{t('concepts.edition.form.action.submit')}
			</PrimaryButton>
		</StyledForm>
	);
};

ConceptEditionForm.propTypes = {
	concept: PropTypes.shape({
		title: PropTypes.string.isRequired,
		description: PropTypes.string.isRequired,
		order: PropTypes.number.isRequired,
	}).isRequired,
	conceptOrderOptions: PropTypes.arrayOf(
		PropTypes.shape({
...
export default ConceptEditionForm;
